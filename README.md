...
- 支持为 HTTPS 仓库单独启用 OAuth2 访问令牌认证。
- 支持为仓库指定分支，留空时使用默认分支。
...
5. 按需填写 **分支**，并启用 **定时拉取**。
6. 如果 HTTPS 仓库需要令牌认证，启用 **访问令牌认证** 并填写 **访问令牌**。
7. 选择 **拉取最新内容** 下载远端更新。
8. 修改笔记后，选择 **上传本地改动**。插件会自动提交、同步远端并上传。
...
- 不建议把访问令牌直接写在 HTTPS URL 中；可以使用 **访问令牌认证** 配置。令牌只保存在本机 Obsidian 插件配置中，并会作为 HTTPS Git 认证密码发送给你配置的远程 Git 服务。
